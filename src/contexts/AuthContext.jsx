...
import { forceLogoutAPI, loginAPI, logoutAPI, getProfileAPI } from "../services/api";
...
export const AuthProvider = ({ children }) => {
  ...
  // Refrescar permisos periódicamente
  useEffect(() => {
        ...
        try {
              ...
              user.canAccessWhatsapp !== freshUser.canAccessWhatsapp) 
          {
            ...
            const updatedUserData = {
              ...user, // Mantiene el token y initials existentes
              ...freshUser, // Sobrescribe con los nuevos datos (incluyendo permisos)
            };
    ...
    }, 60000); // Revisa cada 60 segundos (1 minuto)
  ...
  const logout = async (message = "Sesión cerrada con éxito") => {
    if (user?.token) {
        try {
            await logoutAPI();
        } catch (e) {
