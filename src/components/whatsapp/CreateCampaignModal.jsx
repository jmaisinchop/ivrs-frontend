...
import { DocumentIcon, PhotoIcon, PaperClipIcon } from '@heroicons/react/24/solid';

// Componente interno para la previsualización del mensaje
    ...
    <div className="flex flex-col h-full bg-gray-200 dark:bg-gray-900 rounded-lg p-4 bg-cover" style={{ backgroundImage: "url('/whatsapp_bg.png')" }}>
        <div className="mt-auto max-w-sm ml-auto w-full">
            <div className="bg-green-100 dark:bg-green-800 rounded-lg p-2 shadow-md relative break-words" style={{ borderTopRightRadius: '0px' }}>
                {attachment && (
                    <div className="bg-gray-300 h-32 rounded-md mb-2 flex items-center justify-center overflow-hidden">
...
const CreateCampaignModal = ({ isOpen, onClose, onCampaignCreated }) => {
    ...
    const [attachment, setAttachment] = useState(null); // Para el archivo de imagen/documento (funcionalidad futura)
    const [contactsFile, setContactsFile] = useState(null); // Para el archivo Excel
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('sendDate', new Date().toISOString()); // Se crea con la fecha actual, en estado PAUSADO
        ...
        try {
            ...
            clearForm();
            onCampaignCreated(); // Refresca la tabla de campañas en el componente padre
            onClose();
        } catch (error) {
        ...
        <Dialog open={isOpen} onClose={onClose} className="relative z-50">
            ...
            <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
                <Dialog.Panel className="w-full max-w-4xl rounded-lg bg-white dark:bg-gray-800 p-6 shadow-xl">
                    <Dialog.Title className="text-2xl font-bold text-gray-800 dark:text-white">Crear Nueva Campaña de WhatsApp</Dialog.Title>
