import React, { useState, useEffect, useRef, useContext } from "react"; // Añadido useContext
...
// Definición del map para los pills de estado, movido aquí para acceso global en el archivo si fuera necesario
const statusPillMap = {
...
const StatePill = ({ statusKey }) => { // isDarkMode ya no es necesario aquí si usamos clases dark: directamente
...
const ContactTable = ({ campId }) => {
  ...
  }, [campId, statusFilter]); // Quitado page de aquí, se maneja con el fetch de rows
    ...
    const fetchRows = async () => {
      ...
      try {
        ...
        if (mounted) {
          ...
          if (tableContainerRef.current) tableContainerRef.current.scrollTop = 0; // Scroll to top on data change
        }
      }
    };

    fetchRows();
    const intervalId = setInterval(fetchRows, 2500); // Intervalo un poco más largo
