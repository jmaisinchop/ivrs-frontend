...
import {
  ...
  QuestionMarkCircleIcon, // Se mantiene para el dropdown
  // LifebuoyIcon, // Eliminado
  // ArrowsPointingInIcon, // Eliminado
  // ArrowsPointingOutIcon // Eliminado
} from "@heroicons/react/24/outline";
import { motion, AnimatePresence } from "framer-motion";
import { useTheme } from "../contexts/ThemeContext";
import { SunIcon, MoonIcon } from "@heroicons/react/24/solid";

// Hook para detectar click fuera (sin cambios)
...
const Navbar = ({ sidebarCollapsed, toggleSidebar, autoCollapse, setAutoCollapse }) => { // autoCollapse y setAutoCollapse se mantienen por si el dropdown lo usa
  ...
  return (
    <nav
      className={`fixed top-0 z-40 transition-all duration-300 backdrop-blur-md
                 ...
                 ${scrolled ? "shadow-sm dark:shadow-md" : ""}`} // Ajustado dark:bg y dark:border
      style={{
    ...
    >
      <div className="max-w-full mx-auto px-5">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex md:hidden"> {/* Botón Sidebar Mobile */}
            <motion.button
              ...
              className="inline-flex items-center justify-center p-2 rounded-lg text-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary focus:ring-offset-2 mr-2
                         dark:text-brand-accent dark:focus:ring-offset-dark-900" // Ajustado ring-offset
          ...
          {/* Menú desktop */}
          {/* Los 3 botones de icono han sido eliminados de aquí */}
              ...
              <motion.div whileHover={{ scale: 1.02 }}>
                <button
                  ...
                  className="flex items-center space-x-2 lg:space-x-3 max-w-xs rounded-full focus:outline-none focus:ring-2 
                             focus:ring-brand-primary dark:focus:ring-brand-accent focus:ring-offset-2 dark:focus:ring-offset-dark-900 p-0.5" // Ajustado ring-offset
                ...
                {profileDropdownOpen && (
                  <motion.div
                    ...
                    className="origin-top-right absolute right-0 mt-2 w-64 rounded-xl shadow-2xl py-1 
                               ...
                               divide-y divide-slate-100 dark:divide-slate-700 z-50" // CORRECCIÓN AQUÍ
                  ...
                  >
                    ...
                    <div className="py-1" role="none">
                      {[
                        ...
                        { label: "Ayuda", icon: QuestionMarkCircleIcon, href: "#" }, // Se mantiene este 'Ayuda' dentro del dropdown
                      ].map((item) => (
                        <a
                          key={item.label}
                          href={item.href}
                          role="menuitem"
                          tabIndex={-1} // Correcto para ítems de menú
                          className="flex items-center px-4 py-2.5 text-sm rounded-lg mx-1 transition-colors
                                     text-slate-700 dark:text-slate-200 
                                     hover:bg-slate-100 dark:hover:bg-dark-700 hover:text-brand-primary dark:hover:text-brand-accent" // Ajustado hover dark
                        >
                    ...
                    <div className="py-1" role="none">
                      <div className="flex items-center justify-between px-4 py-2 text-xs text-slate-500 dark:text-slate-400">
                        ...
                        <button
                          onClick={toggleTheme}
                          className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-dark-700 transition" // Ajustado hover dark
                          title={theme === "dark" ? "Modo claro" : "Modo oscuro"}
                          aria-pressed={theme === "dark"}
                          role="menuitemcheckbox" // Correcto
                          tabIndex={-1}
                        >
                          {theme === "dark"
                      ...
                      {/* El toggle de auto-ocultar sidebar se mantiene aquí si se controla desde el dropdown del perfil */}
                      <div className="flex items-center justify-between px-4 py-2 text-xs text-slate-500 dark:text-slate-400">
                        <span>Auto-ocultar sidebar</span>
                        <button
                          onClick={() => setAutoCollapse(!autoCollapse)} // Necesita setAutoCollapse de props
                          className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-dark-700 transition text-brand-primary dark:text-brand-accent" // Ajustado hover dark
                      ...
                      >
                        <LogoutIcon className="h-5 w-5 mr-3 text-red-500" /> {/* Color rojo siempre para este ícono */}
                        Cerrar sesión
                      </button>
                    </div>
                  </motion.div>
