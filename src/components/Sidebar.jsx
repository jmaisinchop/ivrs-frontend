...
import {
  ...
  ShieldCheckIcon, // Icono para la página de Auditoría
...
const Sidebar = ({ user: userProp, collapsed, toggleSidebar, autoCollapse, setAutoCollapse }) => {
  ...
  // Efecto para auto-expansión/contracción con hover (sin cambios)
  useEffect(() => {
  ...
  // Efecto para colapsar por inactividad (sin cambios)
  useEffect(() => {
  ...
  // ✅ **Definición de los items de navegación con sus permisos**
  const navItems = [
    ...
    >
      ...
      <div className="flex-1 flex flex-col overflow-y-auto py-4 px-2 space-y-1">
        <nav className="space-y-1">
          {navItems.map((item) => {
            // ✅ **Lógica de renderizado condicional basada en permisos**
