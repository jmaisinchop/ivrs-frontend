...
import { Modal, Campo, Btn, ToggleSwitch } from './CommonUI';
...
export const CreateUserModal = ({ show, onClose, onUserCreated }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const { username, password, email, firstName, lastName, role } = formulario;
        if (!username || !password || !email || !firstName || !lastName || !role) {
            return toast.warning("Todos los campos marcados con * son obligatorios.");
        }
        ...
        try {
            ...
            onUserCreated(newUser); // Envía el nuevo usuario a la página principal
        ...
        <Modal show={show} onClose={onClose} title="Crear Nuevo Usuario">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Campo label="Nombre(s)*" name="firstName" value={formulario.firstName} onChange={handleChange} required />
                    <Campo label="Apellido(s)*" name="lastName" value={formulario.lastName} onChange={handleChange} required />
                </div>
                <Campo label="Nombre de Usuario*" name="username" value={formulario.username} onChange={handleChange} required />
                <Campo label="Correo Electrónico*" name="email" type="email" value={formulario.email} onChange={handleChange} required />
                <Campo 
                    label="Contraseña*" 
                    name="password" 
                ...
                <div className="flex justify-end pt-4">
                    <Btn type="submit" loading={loading} Icon={UserPlusIcon}>Crear Usuario</Btn>
                </div>
            </form>
        </Modal>
    );
