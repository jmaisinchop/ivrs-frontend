...
import { KeyIcon, PencilSquareIcon, ShieldCheckIcon, TrashIcon } from '@heroicons/react/24/outline';
                        ...
                        <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-dark-700/70 transition-colors duration-100">
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    <div className={`h-10 w-10 rounded-full flex items-center justify-center text-white text-base font-semibold ${u.role === 'ADMIN' ? 'bg-brand-primary' : u.role === 'SUPERVISOR' ? 'bg-brand-secondary' : 'bg-brand-accent'}`}>
                                        {u.firstName?.charAt(0).toUpperCase()}{u.lastName?.charAt(0).toUpperCase()}
                                    </div>
                                    <div className="ml-4">
                                        <div className="text-sm font-medium text-slate-900 dark:text-slate-100">{u.username}</div>
                                        <div className="text-xs text-slate-500 dark:text-slate-400">{u.email}</div>
                                    </div>
                                </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <span className={`px-3 py-1.5 inline-flex items-center text-xs leading-5 font-semibold rounded-full ${getRoleStyles(u.role)}`}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div className="flex space-x-1">
                                    <BtnAction onClick={() => onEdit(u)} title="Editar Usuario"><PencilSquareIcon className="h-5 w-5"/></BtnAction>
                                    <BtnAction onClick={() => onEditPermissions(u)} title="Editar Permisos"><ShieldCheckIcon className="h-5 w-5"/></BtnAction>
                                    <BtnAction onClick={() => onEditPassword(u)} title="Cambiar Contraseña"><KeyIcon className="h-5 w-5"/></BtnAction>
                                </div>
                            </td>
                        </tr>
                    ))}
