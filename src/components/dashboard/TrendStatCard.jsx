...
const TrendStatCard = ({ title, metric, isPercentage = false }) => {
  // Asegurarnos de que metric y sus propiedades existan
  ...
  const getTrend = () => {
    // No mostrar tendencia para valores "en vivo" como llamadas en curso
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md flex flex-col justify-between">
      <div>
        <h3 className="text-gray-500 font-medium truncate">{title}</h3>
        <p className="text-4xl font-bold text-gray-800 mt-2">{displayValue}</p>
      </div>
