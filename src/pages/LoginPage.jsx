...
// Iconos SVG (sin cambios, 'currentColor' es bueno)
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // No necesitas un toast aquí, el AuthContext debería manejar el redirect y los errores globales.
    } finally {
      setIsLoading(false);
    }
  };

  // Asumiendo que tienes 'Inter' cargada globalmente o a través de un import CSS.
  // Si no, reemplaza 'font-inter' con una pila de fuentes de sistema como 'font-sans'.
  return (
    <div className="min-h-screen flex font-inter antialiased bg-slate-100 dark:bg-dark-900 text-slate-900 dark:text-slate-200"> {/* Font Inter y antialiasing */}
      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-brand-primary to-primary-700 items-center justify-center p-12 relative overflow-hidden">
        <div className="absolute inset-0 opacity-[0.04]"> {/* Opacidad sutil para el patrón */}
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <pattern id="subtle-dots" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
                <circle cx="2" cy="2" r="0.5" className="fill-white/50" /> {/* Puntos más sutiles */}
              </pattern>
            </defs>
        ...
        <motion.div 
          initial={{ opacity: 0, y: 20 }} // y:20 para un slide-in sutil
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2, duration: 0.6, ease: "easeOut" }} // easeOut es suave
          className="text-center z-10 max-w-md xl:max-w-lg"
        >
          <div className="flex justify-center mb-8"> {/* Reducido margen inferior */}
            <motion.img
              initial={{ scale: 0.9, opacity: 0 }} // Un poco más grande inicialmente
              animate={{ scale: 1, opacity: 1 }}
              transition={{ delay: 0.4, duration: 0.5, type: "spring", stiffness: 100 }} // Animación de resorte
              src="/fin.png" // Asegúrate que esta ruta sea correcta desde tu carpeta `public`
              alt="Finsolred Logo"
              className="h-24 xl:h-28 object-contain" // Ligeramente más grande
            />
          </div>
          <h1 className="text-4xl xl:text-5xl font-bold text-white mb-5 tracking-tight leading-tight"> {/* tracking-tight para apretar un poco, ajustado leading */}
            Bienvenido al <span className="font-normal text-brand-accent">SISTEMA DE IVRS</span> {/* font-normal para "IVRS" */}
          </h1>
          <p className="text-lg xl:text-xl text-white/80 mb-10 leading-relaxed"> {/* Aumentado leading-relaxed */}
          ...
          <motion.div 
            ...
            className="bg-white/10 dark:bg-black/15 rounded-xl p-6 backdrop-blur-md shadow-xl border border-white/10" // Borde sutil
          >
            <h2 className="text-white font-semibold text-lg mb-4">Novedades del sistema</h2>
            <ul className="text-white/75 text-sm space-y-2.5"> {/* Reducido opacity y espaciado */}
              {[
                ...
                <li className="flex items-start" key={index}>
                  <CheckCircleIcon className="w-5 h-5 mt-0.5 mr-2.5 flex-shrink-0 text-green-400 dark:text-green-300" /> {/* Color ajustado para dark */}
                  {item}
                </li>
              ))}
            </ul>
      ...
      {/* Formulario de login */}
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-8 md:p-10 lg:p-12"> {/* Ajustado padding */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, ease: "easeOut", delay: 0.3 }}
          className="w-full max-w-md p-8 sm:p-10 bg-white dark:bg-dark-800 rounded-xl shadow-xl 
                     border border-slate-200 dark:border-dark-700" // Sombra un poco más suave
        >
          <div className="text-center mb-8"> {/* Reducido margen */}
            <div className="flex justify-center mb-5 lg:hidden"> {/* Logo para móviles */}
              <img
                src="/fin.png"
                alt="Finsolred IVRS Logo"
                className="h-12 object-contain" // Tamaño ajustado
              />
            </div>
          ...
          <form className="space-y-5" onSubmit={handleSubmit}> {/* Espaciado reducido */}
            <div>
              <label htmlFor="username" className="block text-xs font-medium mb-1.5 text-slate-600 dark:text-slate-300 uppercase tracking-wider"> {/* Estilo de etiqueta */}
                Usuario
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-11 pr-4 py-3 rounded-lg border text-sm transition-all duration-200 ease-in-out
                              ...
                              focus:border-brand-primary dark:focus:border-brand-accent`} // Colores de foco actualizados
            ...
            <div className="pt-2"> {/* Añadido padding superior para el botón */}
              <motion.button
                type="submit"
                disabled={isLoading}
                whileHover={{ scale: isLoading ? 1 : 1.02, y: isLoading ? 0 : -1, boxShadow: isLoading ? 'none' : `0 8px 15px -3px var(--color-brand-primary-shadow)` }} // Sombra más sutil
                whileTap={{ scale: isLoading ? 1 : 0.98, y: 0 }}
                transition={{ duration: 0.15, ease: "easeOut" }} // Transición más rápida
                ...
                style={{ '--color-brand-primary-shadow': 'rgba(var(--color-brand-primary-rgb, 5, 79, 120), 0.25)' }} // Usar variable CSS para color de sombra
              >
                {isLoading ? (
          ...
          <div className="mt-8 pt-6 text-center border-t border-slate-200 dark:border-dark-600"> {/* Reducido margen superior */}
