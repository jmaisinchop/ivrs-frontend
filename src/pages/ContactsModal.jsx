...
const ContactsModal = ({ campaignId, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState("manual"); // manual | import | finsolred
  ...
  /* ========== CARGAR PADRES AL ABRIR TAB FINSOLRED ========== */
  useEffect(() => {
  ...
  const handleDragOverInput = (e) => e.preventDefault(); // Renombrado para evitar conflicto con el estado
  /* --- drop EXCEL --- */
  const handleDropExcel = (e) => {
    ...
    setDraggedVariable(null); // Resetear variable arrastrada
  };
    ...
    try {
      ...
      setApiMapping((m) => ({
        ...
        phone:
          cols.find((c) =>
            /telefono|phone|celular|contacto|tel[eé]fono/i.test(c)
          ) || cols[0] || "", // Fallback al primer campo si no hay coincidencia
        name: cols.find((c) => /nombre|name|cliente|persona/i.test(c)) || "",
        identification: cols.find((c) => /identificaci[oó]n|c[eé]dula|ruc|id/i.test(c)) || "",
      }));
  ...
  /* ---------- GUARDA DIRECTO DESDE FINSOLRED ---------- */
  ...
  const handleClose = () => {
    ...
    // No reseteamos rawPadres para no recargarlo si se vuelve a la pestaña
    // setSelectedPadre(""); 
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-500/90 dark:text-slate-400/90 mt-1">
                {activeTab === "manual"
                  ? "Agregue contactos manualmente"
                  : activeTab === "import"
                    ? "Importe contactos desde un archivo"
                    : "Importe contactos desde Finsolred"}
              </p>
            </div>
            <button
              onClick={handleClose}
        ...
        <div className="p-6 overflow-auto flex-1">
          ...
          {activeTab === "manual" && (
            <form onSubmit={handleSubmit} className="space-y-6 h-full flex flex-col">
              <div className="space-y-4 mb-6 flex-1 overflow-y-auto pr-2"> {/* Added pr-2 for scrollbar space */}
                <div className="sticky top-0 z-10 grid grid-cols-12 gap-4 font-medium text-sm text-gray-600 dark:text-slate-300 
                  ...
                  <div className="col-span-3 flex items-center"> {/* Ajustado col-span */}
                  ...
                  <div className="col-span-2 flex items-center"> {/* Nuevo para Identificación */}
                ...
                {contacts.map((c, i) => (
                  ...
                  >
                    <div className="col-span-3"> {/* Ajustado col-span */}
                      <input
                        name="name"
                        value={c.name}
                        onChange={(e) => handleChange(i, e)}
                        placeholder="Nombre completo"
                        className="w-full px-3 py-2 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                   focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                   bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-400"
                      />
                    </div>
                    <div className="col-span-3"> {/* Ajustado col-span */}
                      <input
                    ...
                    <div className="col-span-2"> {/* Nuevo para Identificación */}
                      <input
                        name="identification"
                    ...
                    <div className="col-span-1 flex justify-center">
                      <button
                        ...
                        className="text-gray-400 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 
                                   p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-all disabled:opacity-40"
                      >
              ...
              <div className="flex justify-between items-center border-t border-gray-200/50 dark:border-slate-700/50 pt-6">
                <button
                  type="button"
                  onClick={addRow}
                  className="flex items-center px-4 py-2.5 border border-gray-200/70 dark:border-slate-600/70 
                             rounded-xl hover:bg-gray-50/50 dark:hover:bg-slate-700/50 
                ...
                <div className="space-x-3">
                  <button
                    type="button"
                    onClick={handleClose}
                    className="px-4 py-2.5 border border-gray-200/70 dark:border-slate-600/70 
                               rounded-xl hover:bg-gray-50/50 dark:hover:bg-slate-700/50 
                               text-gray-700 dark:text-slate-200 shadow-xs transition-all"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500
                               text-white rounded-xl hover:shadow-md disabled:opacity-70 flex items-center 
                               shadow-indigo-200 hover:shadow-indigo-300 dark:shadow-indigo-700/50 dark:hover:shadow-indigo-600/50 transition-all"
                  >
                    {isSubmitting ? (
                      <svg
          ...
          {activeTab === "import" && (
            <div className="space-y-6 h-full flex flex-col">
              <div className="flex-1 overflow-y-auto pr-2 space-y-6"> {/* Added pr-2 for scrollbar space and space-y-6 */}
                <div
                  onDragOver={(e) => {
                    e.preventDefault();
                    setDragOver(true);
                  ...
                  className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all ${dragOver
                      ? "border-indigo-400 dark:border-indigo-500 bg-indigo-50/30 dark:bg-indigo-500/10"
                      : "border-gray-300/70 dark:border-slate-600/70 hover:border-indigo-300/80 dark:hover:border-indigo-500/80 " + 
                        "bg-white/50 dark:bg-slate-700/30 hover:bg-indigo-50/20 dark:hover:bg-indigo-500/5"
                ...
                {columns.length > 0 && (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-sm font-medium mb-1.5 text-gray-700/90 dark:text-slate-300/90">
                          Columna teléfono
                        ...
                        <select
                          ...
                          className="mt-1 block w-full p-2.5 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                     focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                      ...
                      <div>
                        ...
                        <select
                          value={mapping.identification}
                          onChange={(e) =>
                            setMapping((m) => ({ ...m, identification: e.target.value }))
                          }
                           className="mt-1 block w-full p-2.5 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                     focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                     bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100"
                    ...
                    <div className="mt-4">
                      <label className="block text-sm font-medium mb-2 text-gray-700/90 dark:text-slate-300/90">
                        Variables disponibles (arrastra a la plantilla)
                      </label>
                    ...
                    <div className="mt-4">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setMapping((m) => ({ ...m, message: e.target.value }))
                        }
                        onDragOver={handleDragOverInput}
                        onDrop={handleDropExcel}
                        className="w-full p-3 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                   focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                   bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-400"
                      />
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="text-sm font-medium text-gray-700/90 dark:text-slate-300/90">
                          Preview ({excelData.length} registros)
                        </h4>
                        <button
                          onClick={() => setShowPreview((v) => !v)}
                          className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 flex items-center transition-all"
                        >
                          {showPreview ? (
                            <>
                              Ocultar
                        ...
                        <div className="overflow-auto max-h-64 border border-gray-200/60 dark:border-slate-600/60 rounded-xl shadow-xs">
                          <table className="min-w-full divide-y divide-gray-200/50 dark:divide-slate-700/50">
                            <thead className="bg-gray-50/50 dark:bg-slate-700/50 sticky top-0 z-10">
                              <tr>
                                <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider">
                                  Nombre
                                </th>
                                <th className="px-4 py-2.5 text-left text-xs font-medium text-gray-600 dark:text-slate-300 uppercase tracking-wider">
                                  Teléfono
                          ...
                          {excelData.length > 5 && (
                            <div className="px-4 py-2.5 text-center text-xs text-gray-500/80 dark:text-slate-400/80 bg-gray-50/50 dark:bg-slate-700/50">
                              Mostrando 5 de {excelData.length} registros
                            </div>
                          )}
                           {excelData.length === 0 && (
                             <div className="px-4 py-8 text-center text-sm text-gray-500/80 dark:text-slate-400/80 bg-white dark:bg-slate-800">
              ...
              {/* Botones de acción para Importar Excel */}
              <div className="flex justify-end space-x-3 border-t border-gray-200/50 dark:border-slate-700/50 pt-6">
                ...
                <button
                  ...
                  disabled={!mapping.phone || !excelData.length || isSubmitting}
                  className="px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500
                             text-white rounded-xl hover:shadow-md disabled:opacity-70 flex items-center 
                             shadow-indigo-200 hover:shadow-indigo-300 dark:shadow-indigo-700/50 dark:hover:shadow-indigo-600/50 transition-all"
                >
          ...
          {activeTab === "finsolred" && (
            <div className="space-y-6 h-full flex flex-col">
              <div className="flex-1 overflow-y-auto pr-2 space-y-6"> {/* Added pr-2 for scrollbar space and space-y-6 */}
                <div>
                  <label className="block text-sm font-medium mb-1.5 text-gray-700/90 dark:text-slate-300/90">
                    Selecciona un “padre” <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={selectedPadre}
                ...
                {selectedPadre && nivelesDisponibles.length > 0 && (
                  <div>
                    <p className="text-sm font-medium mb-2 text-gray-700/90 dark:text-slate-300/90">
                      Carteras disponibles ({nivelesDisponibles.length})
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-auto border border-gray-200/60 dark:border-slate-600/60 rounded-xl p-3 bg-white dark:bg-slate-700/50 shadow-xs">
                      {nivelesDisponibles.map((n) => {
                        ...
                        return (
                          <label
                            key={n}
                            className={`flex items-center space-x-2 px-3 py-2 rounded-lg cursor-pointer transition-all ${ch 
                                ? "bg-indigo-100 dark:bg-indigo-500/30" 
                                : "hover:bg-gray-100/70 dark:hover:bg-slate-600/50"
                              }`}
                          >
                            <input
                              type="checkbox"
                              ...
                              className="text-indigo-600 dark:text-indigo-400 rounded focus:ring-indigo-500 dark:focus:ring-indigo-500 
                                         dark:bg-slate-600 dark:border-slate-500" // Custom checkbox appearance for dark mode
                            />
                    ...
                    <button
                      ...
                      disabled={!nivelesSeleccionados.size || loadingApi}
                      className="mt-4 flex items-center px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500
                                 text-white rounded-xl shadow-indigo-200 hover:shadow-indigo-300 dark:shadow-indigo-700/50 dark:hover:shadow-indigo-600/50 
                ...
                {apiContacts.length > 0 && (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                      <div>
                        <label className="block text-sm font-medium mb-1.5 text-gray-700/90 dark:text-slate-300/90">
                          Campo teléfono<span className="text-red-500 ml-1">*</span>
                        </label>
                        <select
                          value={apiMapping.phone}
                          onChange={(e) =>
                            setApiMapping((m) => ({ ...m, phone: e.target.value }))
                          }
                           className="mt-1 block w-full p-2.5 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                     focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                     bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100"
                      ...
                      <div>
                        ...
                        <select
                          value={apiMapping.name}
                          onChange={(e) =>
                            setApiMapping((m) => ({ ...m, name: e.target.value }))
                          }
                           className="mt-1 block w-full p-2.5 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                     focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                     bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100"
                       ...
                       <div>
                        <label className="block text-sm font-medium mb-1.5 text-gray-700/90 dark:text-slate-300/90">
                          Campo identificación (opcional)
                        </label>
                        <select
                          value={apiMapping.identification}
                          onChange={(e) =>
                            setApiMapping((m) => ({ ...m, identification: e.target.value }))
                          }
                           className="mt-1 block w-full p-2.5 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                     focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                     bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100"
                    ...
                    <div className="mt-4">
                      ...
                      <textarea
                        rows={3}
                        value={apiMapping.message}
                        onChange={(e) =>
                          setApiMapping((m) => ({ ...m, message: e.target.value }))
                        }
                        onDragOver={handleDragOverInput}
                        onDrop={handleDropFinsolred}
                        className="w-full p-3 border border-gray-200/70 dark:border-slate-600/70 rounded-xl 
                                   focus:ring-2 focus:ring-indigo-500/30 dark:focus:ring-indigo-500/50 shadow-xs transition-all
                                   bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-400"
                      />
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="text-sm font-medium text-gray-700/90 dark:text-slate-300/90">
                          Preview (5 de {apiContacts.length})
                        </h4>
                        <button
                          onClick={() => setApiShowPreview((v) => !v)}
                          className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 flex items-center transition-all"
                        >
                      ...
                      {apiShowPreview && (
                        <div className="overflow-auto max-h-64 border border-gray-200/60 dark:border-slate-600/60 rounded-xl shadow-xs">
                          <table className="min-w-full divide-y divide-gray-200/50 dark:divide-slate-700/50">
                            ...
                            <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200/30 dark:divide-slate-700/30">
                              {apiContacts.slice(0, 5).map((r, i) => {
                                ...
                                apiCols.forEach((k) => {
                                  msg = msg.replace(
                                    new RegExp(`{{${k}}}`, "gi"),
                                    String(r[k] === null || r[k] === undefined ? '' : r[k]) // Handle null/undefined values
                                  );
                                });
                                return (
                                  <tr key={i} className="hover:bg-gray-50/30 dark:hover:bg-slate-700/30">
                                    <td className="px-4 py-2.5 text-sm text-gray-700 dark:text-slate-200 whitespace-nowrap">
                                      {apiMapping.name
                                        ? String(r[apiMapping.name] === null || r[apiMapping.name] === undefined ? 'Sin nombre' : r[apiMapping.name])
                                        : "Sin nombre"}
                                    </td>
                                    <td className="px-4 py-2.5 text-sm text-gray-700 dark:text-slate-200 font-medium whitespace-nowrap">
                                     ...
                                     <td className="px-4 py-2.5 text-sm text-gray-700 dark:text-slate-200 whitespace-nowrap">
                                      {apiMapping.identification
                                        ? String(r[apiMapping.identification] === null || r[apiMapping.identification] === undefined ? '' : r[apiMapping.identification])
                                        : ""}
                                    </td>
                                    <td className="px-4 py-2.5 text-sm text-gray-600 dark:text-slate-300 line-clamp-2">
                                      {msg}
                           ...
                           {apiContacts.length === 0 && (
                             <div className="px-4 py-8 text-center text-sm text-gray-500/80 dark:text-slate-400/80 bg-white dark:bg-slate-800">
                                No hay contactos para previsualizar.
                              </div>
                           )}
                        </div>
                      )}
              ...
              {/* Botones de acción para Finsolred */}
               <div className="flex justify-end space-x-3 border-t border-gray-200/50 dark:border-slate-700/50 pt-6">
                <button
                  type="button"
                  onClick={handleClose}
                ...
                <button
                  type="button"
                  onClick={saveContactsFromApi}
                  disabled={isSubmitting || !apiContacts.length || !apiMapping.phone}
                  className="px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 dark:from-indigo-500 dark:to-purple-500
                             text-white rounded-xl hover:shadow-md flex items-center 
                             shadow-indigo-200 hover:shadow-indigo-300 dark:shadow-indigo-700/50 dark:hover:shadow-indigo-600/50 
